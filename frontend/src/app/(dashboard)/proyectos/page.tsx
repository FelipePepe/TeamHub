...
export default function ProyectosPage() {
  ...
  /**
   * Abre el modal de edición cuando se navega con ?editar=<id> desde el detalle.
   */
  useEffect(() => {
    const editarParam = searchParams.get('editar');
    if (!editarParam || !canManageProjects || proyectos.length === 0) return;
    const found = proyectos.find((p) => p.id === editarParam);
    if (!found) return;
    setEditingProyecto(found);
    router.replace('/proyectos', { scroll: false });
  }, [searchParams, proyectos, canManageProjects, router]);
  ...
  let proyectosContent: React.ReactNode;
  if (isLoading) {
    proyectosContent = (
    ...
    proyectosContent = (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        ...
        <Button variant="outline" onClick={() => globalThis.location.reload()} className="mt-4">
          Reintentar
        </Button>
      </div>
    );
  ...
  const proyectoSuffix = filteredProyectos.length === 1 ? '' : 's';
        ...
        <CardHeader>
          ...
          <CardDescription>
            {filteredProyectos.length > 0
              ? `${filteredProyectos.length} proyecto${proyectoSuffix}`
              : 'No hay proyectos'}
          </CardDescription>
        </CardHeader>
        <CardContent>{proyectosContent}</CardContent>
      </Card>

      <ProyectoForm
        open={isCreateOpen || !!editingProyecto}
        onOpenChange={(open) => {
          if (!open) {
