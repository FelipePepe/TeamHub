...
const signMock = vi.fn(async (_method: string, _path: string, _body?: string) => 'sig-123');
vi.mock('../hmac', () => ({ generateRequestSignature: (method: string, path: string, body?: string) => signMock(method, path, body) }));
